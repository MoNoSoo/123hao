...
class simulate extends CI_Controller {
	...
	public function draw_flow_data(){
		...
		$simulateUtil = SimulateUtil::getInstance();
	...
	private function draw_history($etuid){
		...
		foreach ($query->result_array() as $row){
			$row['node_name']  = "未知";
			//处理节点名称
			if(!isNull($row['cs_id']) && $row['cs_id'] > 0){
				$stepId = isNull($row['cs_id'])?"1":$row['cs_id'];
				if($stepId=="1"){
					$nodeName = "草稿";
				}else{
					$wf_xml = XmlUtil::getConfiguration ( $row['wf_uid'] );
					$node = XmlUtil::getElementById($stepId, $wf_xml);
					if(isset($node)){
