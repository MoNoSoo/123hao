...
class route extends CI_Controller {
    ...
    public function query() {
        ...
        while ($row = DBCommon::fetch_array($result)) {
            $row['wf_name'] = wf_iconvutf($row['wf_name']);
            $wflist[] = $row;
        }
        //主流程
        ...
        while ($row = DBCommon::fetch_array($result)) {
            ...
            $row['cs_nodename'] = wf_iconvutf($row['cs_nodename']);
            $row['wf_name'] = wf_iconvutf($row['wf_name']);
    ...
    private function init_wflist($wfid) {
        ...
        $wf['step'] = array();
        $xml = XmlUtil::getConfiguration($wfid);
        ...
        if (isNull($this->wfinfo['route_step']['etuid']) || isNull($this->wfinfo['route_step']['stepid'])) {
            ...
            switch ($type) {
                ...
                case "bystep":
                	...
                	XmlUtil::setGlobal ($this->wfinfo['main_wfid']);//设置主流程全局变量
                    //检查调整环节是否为子流程
                    $wfid = strtoupper($this->wfinfo['route_step']['wfid']);
                    if ($wfid != $this->wfinfo['main_wfid']) {
                    	XmlUtil::setGlobal ($wfid);//设置子流程全局变量
                        $row = $this->wfinfo['wflist'][$wfid];
                        $this->wfinfo['route_step']['p_stepid'] = $row['p_stepid'];
                        $this->wfinfo['route_step']['p_wfid'] = $row['p_wfid'];
                    }
                    ...
                    if ($results->length > 0) {
                        ...
                        if ($xml_result->getAttribute("auto")) {
                            ...
                            //设置工作流必须的系统变量
                            __addGlobalVar('@@createuser__', $this->wfinfo['author_sn']);
                            __addGlobalVar('@@orgcreateuser__', $this->wfinfo['author_sn']);
                            //取审批人
                            $users = XmlUtil::getRoles($role, $this->wfinfo['route_step']['etuid']);
        ...
        try {
            ...
            foreach ($users as $row) {
                ...
                if ($stepid != '500') {
                    ...
                    DBCommon::exec(wf_iconvgbk($insert_sql));
                }
            }
        } catch (Exception $e) {
