...
function node_edit(wfuid,stepid,name) {
	...
	if(configRole){//配置界面
		var html = "<iframe frameBorder='0' width='100%' height='100%' src='index2222.php/config/workflow/wfapprove/role_list?wfuid="+wfuid+"&stepid="+stepid+"'></iframe>";
...
function link_edit(wfuid,from,to,condition_type,condition_value){
	...
	if(condition_type == "config"){//如果是配置项，打开配置页面
		...
		htmlstr += '<a target="_blank" href="index2222.php/config/workflow/list_variate/loadView?wf_uid='+wfuid+'">变量列表</a>';
		htmlstr += '</div>';
		htmlstr += '<div >';
		htmlstr += '<span style="margin-left:10px">判断条件：</span>';
		htmlstr += '</div>';
